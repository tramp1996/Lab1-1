...
public class Read {
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			myPicture.emptyQueue();
			System.out.println("请输入查询最短路径的两个词语:");
			s1 = scanner.nextLine();
			s2 = scanner.nextLine();
			myPicture.shortroad(s1, s2);
			//if (myPicture.calcShortestPath(s1, s2) && myPicture.showShortestPath(s1, s2) != 0)
				//System.out.println("最短路径长度是" + myPicture.minlen);
			String s4 = myPicture.randomWalk();
	...
	public static void show()
	{
		...
		while(true)
		{
			...
			switch(s)
			{
			case "1":scanner.close();return;
			case "2":{
				//System.out.println(myPicture.graph());
				myGraph.createDotGraph(myPicture.graph(), "Graph");
